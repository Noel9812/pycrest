...
async def send_sanction(loan_collection: str, loan_id: str, admin_id: str | None = None):
    ...
    # Doc rule:
    # - <=15L: manager approval is sufficient for sanction generation
    # - >15L: admin approval is required
    if amount > 1500000 and status != LoanStatus.ADMIN_APPROVED:
        raise HTTPException(status_code=400, detail="Admin approval required before sanction letter generation")
    if amount <= 1500000 and status not in [LoanStatus.MANAGER_APPROVED, LoanStatus.ADMIN_APPROVED]:
        raise HTTPException(status_code=400, detail="Manager approval required before sanction letter generation")
    ...
    if not sanction_id:
        ...
        payload = {
            ...
            "mobile_number": (user_doc.get("phone") if user_doc else "") or (kyc_doc.get("phone_number") if kyc_doc else "") or "",
            ...
            "lender_name": loan.get("lender_name") or "Lender / Bank / NBFC Name",
        }
        pdf_bytes = build_sanction_letter_pdf_bytes(payload)
        sanction_id = await store_pdf_document(
            customer_id=loan.get("customer_id"),
            doc_type="sanction_letter",
            filename=f"sanction_letter_loan_{loan.get('loan_id')}.pdf",
            data=pdf_bytes,
        )
